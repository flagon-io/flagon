...
- **CopyButton** (`copy-button.tsx`) — `<CopyButton value variant?>`; icon button that copies to the clipboard with a brief check confirmation.
- **SecretReveal** (`secret-reveal.tsx`) — `<SecretReveal masked onReveal canReveal?>`; a masked value with an eye toggle that reveals on demand via an async `onReveal` (server-side decrypt), then offers copy. **Surfaces failures inline** (e.g. a key created before encryption existed) — never a silent no-op. Use for any stored secret you can read back (SDK keys); PATs / org tokens stay reveal-once.
