// Command flagon is the Flagon platform CLI.
//
// Usage:
//
//	flagon serve      run the HTTP API (migrates on boot, then serves)
//	flagon migrate    provision the app role + run migrations, then exit
//
// Every setting is a flag backed by an environment variable and a sane
// local-dev default, so `go run ./cmd/flagon serve` works against the compose
// Postgres with no setup. For persistent local overrides, drop a `.env` (or
// `.env.local`) next to where you run it - both are gitignored and loaded on
