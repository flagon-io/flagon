...
/**
 * Server-side helpers for the GitHub-style multi-email model. `user_emails` is
 * the source of truth for every address a user has; `users.email` mirrors the
 * one flagged primary (what BetterAuth authenticates against).
 */
...
/**
 * Resolve any VERIFIED address to the owning account's primary email. Sign-in
 * uses this so a user can authenticate with any of their verified addresses:
 * the entered address is mapped to the primary, which is what BetterAuth checks
 * the password against. Unknown or unverified addresses pass straight through
 * (BetterAuth then rejects them like any bad credential), so this never reveals
 ...
 * mirror the new primary onto `users.email`. Guards that the address is the
 * user's and verified.
 */
export async function setPrimaryEmail(
  ...
  const rows = await db
    ...
    .from(userEmails)
    .where(
      and(eq(userEmails.userId, userId), eq(userEmails.email, normalized)),
    )
    .limit(1);
  const target = rows[0];
