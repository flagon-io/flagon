...
export function IntegrationOptions({
  ...
  // What the sender was detected to actually support (null = unknown/legacy).
  const detected = Array.isArray(entry.integration?.config?.detected)
    ? (entry.integration.config.detected as string[])
    : null;
  const supports = (gates?: string) =>
    !gates || !detected ? true : detected.includes(gates);
  ...
  return (
    <div className="flex flex-col divide-y divide-white/8">
      {entry.options.map((o) => {
        const usable = supports(o.gatesCapability);
        return (
            ...
            <div className="min-w-0">
              ...
              {!usable ? (
                <p className="mt-1 text-xs text-amber-300">
                  Your {entry.label} sender doesn&apos;t support this, so it can&apos;t be turned on.
                </p>
              ) : null}
            </div>
            <Switch
              checked={usable ? (values[o.key] ?? o.default) : false}
              onCheckedChange={(next) => toggle(o.key, next)}
              disabled={!canManage || pending || !usable}
              ariaLabel={o.label}
            />
          </div>
        );
