...
/**
 * One incident: severity/status, the affected services, the timeline, the runbook
 * checklist, and the resolve control. Any member acts; only managers edit structural
 * metadata (via the controls the client gates).
 */
export default async function IncidentPage({ params }: { params: Promise<{ org: string; number: string }> }) {
  ...
  const [detail, orgMembers, projects, runbooks, levels] = await Promise.all([
