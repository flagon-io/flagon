...
export function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-xl border border-white/10 bg-white/2 ${className ?? ""}`}>
      {children}
    </section>
  );
}
...
export function CardHead({ title, action }: { title: string; action?: React.ReactNode }) {
  return (
    <div className="flex min-h-[52px] items-center justify-between gap-2 border-b border-white/8 px-4 py-3">
...
export function EmptyPrompt({
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      {action ? (
        <Link href={action.href} className="shrink-0 text-sm text-teal-400 hover:text-teal-300">
          {action.label}
        </Link>
      ) : null}
    </div>
