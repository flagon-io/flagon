"use client";

import type { ReactNode } from "react";
import { IconGitHub } from "@flagon/design";
import { authClient } from "@/lib/auth-client";
import type { OAuthProviders } from "@/lib/oauth";

/**
 * The social sign-in row (Google, GitHub), modeled on GitHub's own. Buttons for
 * unconfigured providers render DISABLED rather than hidden, so the form's shape
 * is stable and every provider always occupies its place. A configured button
 * hands off to BetterAuth's social sign-in.
 */
export function SocialButtons({ providers }: { providers: OAuthProviders }) {
  async function go(provider: "google" | "github") {
    await authClient.signIn.social({ provider, callbackURL: "/" });
  }

  return (
    <div className="flex flex-col gap-2">
      <SocialButton
        label="Continue with Google"
        icon={<GoogleIcon />}
        enabled={providers.google}
        onClick={() => go("google")}
      />
      <SocialButton
        label="Continue with GitHub"
        icon={<IconGitHub className="h-4 w-4" />}
        enabled={providers.github}
        onClick={() => go("github")}
      />
    </div>
  );
}

function SocialButton({
  label,
  icon,
  enabled,
  onClick,
}: {
  label: string;
  icon: ReactNode;
  enabled: boolean;
  onClick: () => void;
}) {
  return (
    <button
      type="button"
      onClick={enabled ? onClick : undefined}
      disabled={!enabled}
      title={enabled ? undefined : "Coming soon"}
      aria-disabled={!enabled}
      className="flex h-10 items-center justify-center gap-2.5 rounded-md border border-white/10 bg-white/5 text-sm font-medium text-zinc-200 transition-colors hover:border-white/20 hover:bg-white/8 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:border-white/10 disabled:hover:bg-white/5"
    >
      {icon}
      {label}
    </button>
  );
}

/** The four-color Google "G". Inlined so the buttons need no external asset. */
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden focusable="false">
      <path
        fill="#FFC107"
        d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"
      />
      <path
        fill="#FF3D00"
        d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.611 20.083H42V20H24v8h11.303c-.792 2.237-2.231 4.166-4.087 5.571.001-.001.002-.001.003-.002l6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"
      />
    </svg>
  );
}
