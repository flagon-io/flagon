...
/**
 * Small stateless signed tokens for out-of-band flows (secondary-email
 * verification), where BetterAuth's own verification store does not apply. A
 * token is `base64url(payload).base64url(hmac)`, signed with the auth secret.
 * Stateless: nothing to store; tampering/expiry is caught on verify. Ported from
 * apps/app/src/lib/token.ts (kept identical).
 */
