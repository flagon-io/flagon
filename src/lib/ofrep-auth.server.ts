...
export async function authenticateOfrep(request: Request, allowClient: boolean) {
  ...
  if (access) {
    ...
    const [membership] = await db.select({ id: members.id }).from(members).where(and(
  ...
  return { orgId: client.organizationId, kind: "client" as const, origin: request.headers.get("origin") };
}
