...
describe("OFREP response caching", () => {
  ...
  it("makes configuration versions deterministic and sensitive to segments", () => {
    expect(configurationVersion([flag], [segment])).toBe(
      configurationVersion([flag], [segment]),
    );
    expect(configurationVersion([flag], [segment])).not.toBe(
      configurationVersion(
        [flag],
        [{ ...segment, updatedAt: new Date("2026-01-02T00:00:00Z") }],
      ),
    );
  });
  ...
  it("uses stable context ordering but separates different targeting contexts", () => {
    ...
    expect(stableJson({ plan: "pro", targetingKey: "a" })).toBe(
      stableJson({ targetingKey: "a", plan: "pro" }),
    );
    expect(evaluationEtag(version, { plan: "pro", targetingKey: "a" })).toBe(
      evaluationEtag(version, { targetingKey: "a", plan: "pro" }),
    );
    expect(evaluationEtag(version, { targetingKey: "a" })).not.toBe(
      evaluationEtag(version, { targetingKey: "b" }),
    );
  });
  ...
  it("parses bounded JSON bodies even without a content-length header", async () => {
    const valid = await readOfrepJson(
      new Request("https://example.test", {
        method: "POST",
        body: JSON.stringify({ context: { targetingKey: "a" } }),
      }),
    );
    expect(valid.ok).toBe(true);
    const oversized = await readOfrepJson(
      new Request("https://example.test", {
        method: "POST",
        body: "x".repeat(OFREP_MAX_BODY_BYTES + 1),
      }),
    );
