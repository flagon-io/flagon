...
describe.skipIf(!enabled)("stripe sandbox", () => {
  ...
  it("bills only an enterprise org's METERED overage, not its covered usage", async () => {
    const secret = "whsec_enterprise_test_secret";
    process.env.STRIPE_WEBHOOK_SECRET = secret;
    ...
    const slug = `wh-ent-${Date.now()}`;
    let orgId = "";

    try {
      const [org] = await owner`
        INSERT INTO organizations (slug, name, plan, stripe_customer_id)
        VALUES (${slug}, 'Enterprise Org', 'enterprise', ${customerId})
        RETURNING id`;
      orgId = org.id as string;

      // Evaluations are COVERED by the contract envelope; syncs are METERED
      // with 1M included per cycle.
      await owner`
      ...
      // Big covered usage (would be $500 if billed) and 3M syncs = 2M overage
      // past the 1M included, at $0.75/1M = $1.50.
      await owner`
        INSERT INTO usage_rollups (organization_id, meter, day, quantity) VALUES
          (${orgId}::uuid, 'flags.evaluations', '2026-05-10'::date, 500000000),
          (${orgId}::uuid, 'flags.syncs', '2026-05-10'::date, 3000000)`;
      ...
      // ONLY the metered overage ($1.50) lands: the covered evaluations, worth
      // $500 at list, are coordinated at renewal and never auto-charged.
      const after = await stripe.invoices.retrieve(invoice.id as string);
      expect(after.total).toBe(150);

      // The period is still closed+invoiced with the covered line frozen at 0.
      const [period] = await owner`
        SELECT status FROM billing_periods
        WHERE organization_id = ${orgId}::uuid ORDER BY period_start DESC LIMIT 1`;
      expect(period.status).toBe("invoiced");
      const [covered] = await owner`
        SELECT billing_mode, cost_cents FROM billing_period_lines l
        JOIN billing_periods p ON p.id = l.billing_period_id
        WHERE p.organization_id = ${orgId}::uuid AND l.meter = 'flags.evaluations'`;
      expect(covered.billing_mode).toBe("covered");
      expect(Number(covered.cost_cents)).toBe(0);
      ...
      expect(afterRedelivery.total).toBe(150);
    } finally {
      if (orgId)
        await owner`DELETE FROM organizations WHERE id = ${orgId}::uuid`;
      await owner.end();
