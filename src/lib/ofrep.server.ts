...
export function configurationVersion(
  flags: VersionedResource[],
  segments: VersionedResource[],
) {
  const entries = [
...
export function etagMatches(header: string | null, current: string) {
  ...
  return header
    ...
    .some(
      (candidate) =>
        candidate.trim() === "*" || normalize(candidate) === normalize(current),
    );
}

export function validEvaluationContext(
  value: unknown,
): value is Record<string, unknown> & { targetingKey: string } {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const targetingKey = (value as Record<string, unknown>).targetingKey;
  return (
    typeof targetingKey === "string" &&
    targetingKey.trim().length > 0 &&
    targetingKey.length <= 1024
  );
}
...
export const OFREP_HEADERS = {
  ...
  "Access-Control-Allow-Headers":
    "Authorization, Content-Type, If-None-Match, X-Flagon-Organization, X-Flagon-Event-Id, Idempotency-Key",
...
export async function readOfrepJson(
  request: Request,
