...
export default function AuthenticationDocsPage() {
  return (
    <div className="min-w-0">
      ...
      <p className={p}>
        A {brand.name} account is a username, one or more email addresses, and a
        password. This page covers how accounts, email verification, sessions,
        and passwords behave, and how authentication applies to the API.
      </p>
      ...
      <p className={p}>
        ...
        with a hyphen, and are between 2 and 39 characters. You can change your
        username at any time in{" "}
      ...
      <p className={p}>
        At sign-in, one field accepts either your username or any verified email
        address on your account, plus your password.
      </p>
      ...
      <p className={p}>
        ...
        email in use on any account (primary or not) cannot be added to another.
      </p>

      <h3 className={h3}>Verification</h3>
      <ul className="mt-3 list-disc space-y-2 pl-5">
        <li className={li}>
          Signing up sends a verification link to your address. The link expires
          in 1 hour; you can resend it from the banner in the console or from
          email settings.
        </li>
        <li className={li}>
          You can sign in with an unverified primary email. Locking you out of
          the account that could resend the verification would be worse, so the
          console shows a persistent reminder instead.
        </li>
        <li className={li}>
      ...
      <p className={p}>
        Signing in creates a browser session that spans the whole platform (the
        app, the marketing site, and the API share it on{" "}
        <span className={code}>*.{brand.domain}</span>). The{" "}
        <Link href={appHref("/settings/sessions")} className={a}>
          sessions page
        </Link>{" "}
        lists every device signed in to your account with its browser, platform,
        IP address, and sign-in time; you can revoke any single session or sign
        out everywhere else in one click.
      </p>
      ...
      <ul className="mt-3 list-disc space-y-2 pl-5">
        ...
        <li className={li}>
          Forgot it? The reset flow emails a single-use link (expires in 1 hour)
          to your primary address, whichever of your account&apos;s emails you
          typed into the form.
        </li>
      </ul>
      ...
      <p className={p}>
        Account deletion is self-service in the danger zone of account settings,
        confirmed by typing your username and password. Deletion is permanent
        and removes your data from {brand.name}.
      </p>

      <h2 className={h2}>Authenticating with the API</h2>
      <p className={p}>
        The API is designed around access tokens:{" "}
        <em>personal access tokens</em> that act as you, and{" "}
        <em>organization access tokens</em> for automation owned by an org, both
        sent as a Bearer token in the{" "}
        <span className={code}>Authorization</span> header. Token issuance opens
        with the Access tokens settings page; until then, requests from a
        signed-in browser authenticate with the session cookie (which is how the
        interactive console in the API reference works). Account security
