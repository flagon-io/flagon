...
import { createClientToken, listClientTokens, serializeClientToken } from "@/lib/client-tokens.server";
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  return apiJson((await listClientTokens(context.org.id)).map(serializeClientToken));
}
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const result = await createClientToken(context.org.id, typeof body?.name === "string" ? body.name : "");
  if (!result.ok) return apiError(400, "invalid_token", result.error);
  return apiJson({ ...serializeClientToken(result.clientToken), token: result.token }, { status: 201 });
}
