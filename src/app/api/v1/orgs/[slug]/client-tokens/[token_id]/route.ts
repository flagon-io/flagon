...
export async function PATCH(
  request: Request,
  { params }: { params: Promise<{ slug: string; token_id: string }> },
) {
  ...
  return result.ok
    ? apiJson(serializeClientToken(result.clientToken))
    : apiError(404, "not_found", "Client token not found.");
}
export async function DELETE(
  ...
  return (await revokeClientToken(context.org.id, id))
    ? apiNoContent()
    : apiError(404, "not_found", "Client token not found.");
}
