...
import { deleteFlag, getFlag, serializeFlag, updateFlag } from "@/lib/flags.server";
...
export async function GET(request: Request, { params }: Params) {
  ...
  return flag ? apiJson(serializeFlag(flag)) : apiError(404, "not_found", "Flag not found.");
}
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  if (!body || typeof body !== "object") return apiError(400, "invalid_flag", "Provide fields to update.");
  const result = await updateFlag(context.org.id, key, {
    ...
    defaultVariant: typeof body.default_variant === "string" ? body.default_variant : undefined,
    rules: Array.isArray(body.rules) ? body.rules : undefined,
  });
  if (!result.ok) return apiError(result.code === "not_found" ? 404 : 400, result.code, result.error);
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  const context = await resolveFlagOrg(request, slug, "flags:write");
  if ("error" in context) return context.error;
  return await deleteFlag(context.org.id, key) ? apiNoContent() : apiError(404, "not_found", "Flag not found.");
}
