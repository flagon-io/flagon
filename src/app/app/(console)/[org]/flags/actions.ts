...
import { coerceValue, defaultVariants, FLAG_TYPES, type FlagType, type TargetingRule, type Variant } from "@/lib/flags";
...
export async function createFlagAction(orgSlug: string, form: FormData) {
  ...
  const type: FlagType = FLAG_TYPES.includes(requested as FlagType) ? requested as FlagType : "boolean";
  ...
  try {
    if (type === "boolean") {
      ...
      const drafts = JSON.parse(String(form.get("variants") ?? "[]")) as Array<{ key?: unknown; raw?: unknown }>;
      variants = drafts
        .filter((draft) => String(draft?.key ?? "").trim())
        .map((draft) => {
          const key = String(draft.key).trim();
          const value = String(draft.raw ?? "");
          return { key, value: type === "object" ? JSON.parse(value || "{}") : coerceValue(type, type === "integer" || type === "float" ? Number(value || 0) : value) };
        });
      ...
      if (!variants.some((variant) => variant.key === defaultVariant)) defaultVariant = variants[0].key;
    }
  } catch {
    return;
  }
  const result = await createFlag(org.id, { name: String(form.get("name") ?? ""), key: String(form.get("key") ?? ""), description: String(form.get("description") ?? ""), type, variants, defaultVariant });
  if (!result.ok) return;
  refresh(orgSlug);
  redirect(appPath(`/${orgSlug}/flags/${result.flag.key}`));
}

export async function setDefaultVariantAction(orgSlug: string, key: string, variant: string) {
  const org = await resolveOrg(orgSlug);
  if (!org) return;
...
export async function saveFlagDefinitionAction(orgSlug: string, key: string, form: FormData) {
  const org = await resolveOrg(orgSlug);
  if (!org) return;
  try {
    const variants = JSON.parse(String(form.get("variants") ?? "[]")) as Variant[];
    const rules = JSON.parse(String(form.get("rules") ?? "[]")) as TargetingRule[];
...
export async function deleteFlagAction(orgSlug: string, key: string, form: FormData) {
