...
describe.skipIf(!canRun)("organization feature flags", () => {
  ...
  afterAll(async () => {
    if (owner) {
      if (orgId)
        await owner`DELETE FROM access_tokens WHERE subject_type = 'organization' AND subject_id = ${orgId}`;
  ...
  it("creates typed flags, segments, and both credential classes under RLS", async () => {
    ...
    const [org] =
      await owner`INSERT INTO organizations (slug, name) VALUES (${slug}, 'Flags') RETURNING id`;
    orgId = org.id as string;
    ({ closePool } = await import("@/db/client"));
    const { createFlag, getFlag, updateFlag } =
      await import("@/lib/flags.server");
    const { createSegment } = await import("@/lib/segments.server");
    const { createAccessToken, authenticateAccessToken } =
      await import("@/lib/access-tokens.server");
    const { createClientToken, authenticateClientToken } =
      await import("@/lib/client-tokens.server");
    expect(
      (await createFlag(orgId, { key: "new-checkout", name: "New checkout" }))
        .ok,
    ).toBe(true);
    expect((await getFlag(orgId, "new-checkout"))?.defaultVariant).toBe("off");
    const updated = await updateFlag(orgId, "new-checkout", {
      defaultVariant: "on",
    });
    expect(updated.ok && updated.flag.defaultVariant).toBe("on");
    expect(
      (
        await createSegment(orgId, {
          key: "beta",
          name: "Beta",
    ...
    const server = await createAccessToken({
    ...
    if (server.ok)
      ...
      ).toMatchObject({ subjectId: orgId });
    const client = await createClientToken(orgId, "iOS app");
    expect(client.ok).toBe(true);
    if (client.ok)
      expect(
        await authenticateClientToken(`Bearer ${client.token}`),
      ).toMatchObject({ organizationId: orgId });
    if (!server.ok || !client.ok)
      throw new Error("Evaluation credentials were not created.");

    const { POST: bulkEvaluate } =
      await import("@/app/api/ofrep/v1/evaluate/flags/route");
    const bulkRequest = (etag?: string) =>
      new Request("https://api.flagon.io/ofrep/v1/evaluate/flags", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          context: { targetingKey: "browser-demo", environment: "production" },
        }),
      });
    ...
    expect((await firstBulk.json()).flags).toMatchObject([
      { key: "new-checkout", value: true },
    ]);
    const etag = firstBulk.headers.get("etag");
    expect(etag).toBeTruthy();
    expect((await bulkEvaluate(bulkRequest(etag ?? undefined))).status).toBe(
      304,
    );

    const { POST: singleEvaluate } =
      await import("@/app/api/ofrep/v1/evaluate/flags/[key]/route");
    const single = await singleEvaluate(
      new Request(
        "https://api.flagon.io/ofrep/v1/evaluate/flags/new-checkout",
        {
          ...
          body: JSON.stringify({ context: { targetingKey: "server-request" } }),
        },
      ),
    ...
    // work nobody did.
    const revalidated = await singleEvaluate(
      new Request(
        "https://api.flagon.io/ofrep/v1/evaluate/flags/new-checkout",
        {
          method: "POST",
    ...
    const clientSingle = await singleEvaluate(
      new Request(
        "https://api.flagon.io/ofrep/v1/evaluate/flags/new-checkout",
        {
          method: "POST",
          ...
          body: JSON.stringify({ context: { targetingKey: "browser-single" } }),
        },
      ),
      { params: Promise.resolve({ key: "new-checkout" }) },
    );
    expect(clientSingle.status).toBe(200);
    expect(await clientSingle.json()).toMatchObject({
      key: "new-checkout",
      value: true,
    });

    // A DIFFERENT context is a different representation, so it is not a hit.
    const otherContext = await singleEvaluate(
      new Request(
        "https://api.flagon.io/ofrep/v1/evaluate/flags/new-checkout",
        {
          method: "POST",
          ...
          body: JSON.stringify({ context: { targetingKey: "someone-else" } }),
        },
      ),
      { params: Promise.resolve({ key: "new-checkout" }) },
    );
    ...
    // 304s did not.
    const [pending] =
      await owner`SELECT count(*)::int AS count, COALESCE(sum(quantity), 0)::int AS quantity FROM usage_events WHERE organization_id = ${orgId} AND meter = 'flags.evaluations'`;
    ...
    const [syncs] =
      await owner`SELECT COALESCE(sum(quantity), 0)::int AS quantity FROM usage_events WHERE organization_id = ${orgId} AND meter = 'flags.syncs'`;
    expect(syncs.quantity).toBe(1);

    const { compactUsageEvents } = await import("@/lib/usage-events.server");
    await compactUsageEvents({ orgId });
    const [metered] =
      await owner`SELECT quantity FROM usage_rollups WHERE organization_id = ${orgId} AND meter = 'flags.evaluations'`;
    expect(Number(metered.quantity)).toBe(4);
    const [meteredSyncs] =
      await owner`SELECT quantity FROM usage_rollups WHERE organization_id = ${orgId} AND meter = 'flags.syncs'`;
    expect(Number(meteredSyncs.quantity)).toBe(1);
    const { listClientTokens, rotateClientToken } =
      await import("@/lib/client-tokens.server");
    ...
    if (rotatedClient.ok)
      expect(
        await authenticateClientToken(`Bearer ${rotatedClient.token}`),
      ).toMatchObject({ id: client.clientToken.id });

    const { rotateAccessToken } = await import("@/lib/access-tokens.server");
    const rotatedServer = await rotateAccessToken(
      "organization",
      orgId,
      server.accessToken.id,
    );
